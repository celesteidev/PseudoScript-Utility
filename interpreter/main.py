...
def execute_psu_script(filepath):
        ...
        try:
            ...
            # Command: card
                ...
                if match:
                    ...
                    # Remove the class attribute if it exists to avoid duplication
                    filtered_attrs = attr_str_html.replace(f'class="{card_class}"', '')
                    append_html_line(f'<div class="psu-card {card_class}" {filtered_attrs}>'.strip())
                    # Card header
    ...
    # --- Output the HTML directly to stdout (like PHP) ---
    try:
        final_html_content = '\n'.join(html_output_buffer)
        print(final_html_content)  # Output directly to console/browser
        print(f"--- PSU Execution Completed ---", file=sys.stderr)
    except Exception as e:
